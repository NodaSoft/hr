...
func (m *TaskManager) Run () {
	...
	m.results = map[int]Task{}								// Мапа результатов по id таска
...
// Горутина ничего не делает, поэтому не нужно её и учитывать в wg
// Для теста этой функции закомментируйте в Sender условие for /*...*/ { ... }
// Не беспокойтесь о переполнении памяти и длинном выводе, память контролируется мьютексом, а вывод ограничен.
// В отладчике ввод с клавиатуры может не сработать, тогда соберите $ go build .
...
func (m *TaskManager) Log () {
	...
	fmt.Println("\x1b[32m" + "Done tasks: ",len(m.results))			
		// Список успешных результатов зеленым цветом
	i:=0
...
func main() {
	/** Создаем Менеджера тасков с параметрами. Все параметры опциональны, устанавливаются в Run
	 *  Показано, что число итераций больше буфера, и это не приводит к сбою.
	 *  Количество тасков равняется iCount либо бесконечно, но параллелизм ограничен goMax
	 *  goMax ограничен системными лимитами, корректируется в Run.
	 *  Чтобы при бесконечном цикле goMax установился на максимум, его нужно задать явно любым очень большим числом
	 */
	m := TaskManager {
